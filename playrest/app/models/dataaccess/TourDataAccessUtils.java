...
public class TourDataAccessUtils {
	 ...
	 * @throws IllegalArgumentException if the user or the sport declared in the
	 * given DTO were not found.
	 */
	public static int create(TourDto tourDto) {
	 ...
	 * not found.
	 */
	public static TourDto findById(int id) {
	 ...
	 */
	public static List<TourDto> findByUsername(String username) {
	...
	/**
	 * @return A list of tours created for the specified {@code sport}.
	 * If none are found, an empty list is returned.
	 */
	public static List<TourDto> findBySportName(String sport) {
	 ...
	 * ({@code lat, lon, alt}).
	 * 
	 * @param lat The WGS84 latitude in degrees.
	 * @param lon The WGS84 longitude in degrees.
	 * @param alt An optional altitude above sea level in metres.
	 * @param radius The radius in metres.
	 * @param sport The optional additional sport filter.
	 * 
	 * @return A list of tours satisfying the provided criteria. If none are
	 * found, an empty list is returned.
	 */
	public static List<TourDto> findByStartPoint(
			double lat,
			double lon,
			Optional<Double> alt,
			double radius,
		...
		LOG.debug("Fetching tours for criteria: lat=" + 
			lat + " lon=" + lon + " alt=" + alt.or(0d) + " radius=" + radius +
			" " + sport.or(""));
	 ...
	 * @throws IllegalArgumentException if the user or the sport declared in
	 * the given DTO were not found.
	 */
	static Tour toEntity(TourDto tourDto) {
		...
		List<TimestampedPoint> tsPoints =
				Lists.newArrayListWithExpectedSize(tourDto.points.size());
	...
	static TourDto toDto(Tour tour) {
		...
		List<TimestampedPointDto> points =
				Lists.newArrayListWithExpectedSize(tour.getPoints().size());
		for (TimestampedPoint tsPoint : tour.getPoints()) {
			...
			TimestampedPointDto tsPointDto =
					new TimestampedPointDto(time, x, y, z);
